...
//help:Presentation(texfile,txtfile);
  regional(packL,file,flgL,flg,verbflg,slideL,ns,slideorgL,
     txL,wall,sld,slidecmd,tmp,tmp0,tmp1,
     tmp2,tmp3,tmp4,tmp5,newoption,top,repeatflg,nrep,nrepprev,
     repstr,sestr,npara,paradt,parafiles,hyperflg,paractr,
...
//  SCEOUTPUT = openfile(file);
  ...
  if(GPACK=="tpic", //260227from
      txL=append(txL,"\usepackage{ketpic,ketlayer}");  
      txL=append(txL,
         "\special{papersize=\the\paperwidth,\the\paperheight}");
  );
  if(GPACK=="pict2e",
    txL=append(txL,"\usepackage{pict2e}");
    txL=append(txL,"\usepackage{ketpic2e,ketlayer2e}");
  );
  if(GPACK=="tikz",
    txL=append(txL,"\usepackage{tikz}");
    txL=append(txL,"\usepackage{pict2e}");
    txL=append(txL,"\usepackage{ketpic2e,ketlayer2e}");
  );
  ...
  if(indexof(PathT,"lualatex")>0,//260227
    txL=append(txL,"\usepackage{geometry}");
    txL=append(txL,"\geometry{paperwidth=141mm,paperheight=91mm}");
    txL=append(txL,"\usepackage{luatexja}");
    txL=append(txL,"\usepackage{ketpic2e,ketlayer2e}");
  );
  if(length(wall)==0, //240412from
    txL=append(txL,"\usepackage{ketslide}");
  ,
    txL=append(txL,"\usepackage{ketslide2}");
  );
  txL=append(txL,"\usepackage{amsmath,amssymb}");
  txL=append(txL,"\usepackage{bm,enumerate}");
  if((indexof(PathT,"pdflatex")>0)%(indexof(PathT,"lualatex")>0),
...
//  tmp3=tmp3_tmp;
  ...
  forall(1..15,
    ...
    if(islist(tmp),
      ...
      if(length(SlideColorList_#)==3,
        txL=append(txL,
          "\definecolor{slidecolor"+tmp4_#+"}{rgb}{"+tmp+"}");
      );//17.01.07until
      SlideColorList_#="slidecolor"+tmp4_#;
    );
  );
  txL=append(txL,"\def\setthin#1{\def\thin{#1}}");//17.08.23
  txL=append(txL,"\setthin{"+text(ThinDense)+"}");//17.08.23
  txL=append(txL,"\newcounter{pagectr}");
  txL=append(txL,"\setcounter{pagectr}{1}");
  txL=append(txL,"\renewcommand{\slidepage}[1][\monthday-]{%");//260224from
  txL=append(txL,"\setcounter{ketpicctra}{18}%");
  txL=append(txL,"");//180908
  txL=append(txL,"\begin{layer}{118}{0}");
  txL=append(txL,"\putnotew{130}{-\theketpicctra.05}{\small#1\thepage/\pageref{pageend}}");
  txL=append(txL,"\end{layer}");
  txL=append(txL,"");//180908
  txL=append(txL,"}");//189524to
  ...
  txL=append(txL,tmp);
  txL=append(txL,"");
  txL=append(txL,"\ketslideinit");
  txL=append(txL,"");
//  forall(tmp, // 15.07.21
//    if(substring(#,0,1)=="\", txL=append(txL,#));
//  );
  txL=append(txL,"\setmargin{9}{9}{3}{3}"); //260227
  txL=append(txL,"\pagestyle{empty}");//180524to
  txL=append(txL,"");
  txL=append(txL,"\begin{document}");
  txL=append(txL,"");
  if(length(wall)>0,
    txL=append(txL,"\input{fig/"+wall+".tex}");
    txL=append(txL,"");
  );
  if(length(tmp2)>0,
    if(indexof(tmp2," no")==0, // 16.11.11from
      txL=append(txL,"\begin{layer}{120}{0}");
      if(substring(tmp2,0,1)!="\",
        tmp2="\putnotese{0}{0}{\input{fig/"+tmp2+".tex}}";
        //16.12.27
      );
      txL=append(txL,tmp2);
      txL=append(txL,"\end{layer}");
    ...
    ,
      ...
      txL=append(txL,"");
      txL=append(txL,"\vspace*{"+top+"}");
      txL=append(txL,"");
    ); // 16.11.11until
  );
  txL=append(txL,"\def\mainslidetitley{22}");
  ...
  txL=append(txL,"");
  txL=append(txL,"\color{"+SlideColorList_(15)+"}");
  txL=append(txL,BodyStyle);//17.01.07
  txL=append(txL,"\addtocounter{page}{-1}");//17.01.29
  txL=append(txL,"");
  verbflg=0; //16.06.28
  ...
  forall(2..length(slideL),ns,
    ...
    if(flg==0,  
      ...
      if(flg==0&tmp1=="same",
        ...
        txL=append(txL,tmp);
        txL=append(txL,""); //180524
        txL=append(txL,"\vspace*{18mm}");
        txL=append(txL,"");
        tmp4=tmp3;
        tmp3=tmp2;
        tmp2="";
  ...
  txL=append(txL,"\label{pageend}\mbox{}"); //180529
  txL=append(txL,"");
  txL=append(txL,"\end{document}");
  SCEOUTPUT=openfile(file);
  forall(txL,
    println(SCEOUTPUT,#);
  );
...
//help:Maketitle();
  ...
  println(SCEOUTPUT,"\geometry{paperwidth=141mm,paperheight=91mm}");
  ...
  println(SCEOUTPUT,"\usepackage{graphicx}");
  println(SCEOUTPUT,"\usepackage{color}");
  println(SCEOUTPUT,"\setmargin{9}{9}{3}{3}");
